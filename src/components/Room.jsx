...
const Room = ({tick}) => {
    ...
    // Handles click of a square on the grid. Sets target square to be the clicked one
    ...
    // Handles events that occur every tick. Reduces player attack cooldown and
    // keeps track of olms state
    useEffect(() => {
        if(path && path.length > 0) move()
    ...
    const move = () => {
        ...
        updatedSquares[location.row][location.col] = !updatedSquares[location.row][location.col]
        updatedSquares[path[0].row][path[0].col] = !updatedSquares[path[0].row][path[0].col]
    ...
    // Handles which olm image to show based on the olmDirection
    const Olm = () => {
        switch(olmDirection) {
            case 'left':
                return (
